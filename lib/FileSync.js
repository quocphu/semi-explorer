  ...
  self.prepareToSync(opts, function(err) {
    ...
    async.whilst(
      ...
      function(w_cb) {
        ...
        // Get block from file
        self.syncFile(self.currentFile, function(err, blockCount){
          if (err) {
          ...
          console.log('Finish: %d, block: %d, time: %d', self.currentFile - 1, count, time/1000);
          w_cb();
        });
      }, function(err){
          ...
          async.whilst(
            ...
            function(cbTx){
              async.setImmediate(function(){
                ...
                parsedTx.time = tmpBlock.timestamp;
            ...
            function (err){
              ...
              self.addressStore.pushAll(self.db, function(pushError, count){
                console.log('File=%d, txcount=%d ', self.currentFile, txCount,"\t\t", (new Date().getTime() - start)/1000);
