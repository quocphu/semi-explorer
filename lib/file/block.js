...
Block.prototype.getHeader = function(hashBlockFunction){
        var buffer = this.toBuffer(true);
        var blockHash = '';
        if(typeof hashBlockFunction == 'function') {
            blockHash = hashBlockFunction(buffer);
        } else {        
            blockHash = [].reverse.call(bitcoinjs.crypto.hash256(buffer)).toString('hex');
        }
        
        var block = Block.fromBuffer(buffer);   
        block.previousblockhash = [].reverse.call(block.prevHash).toString('hex');
        block.merkleroot = [].reverse.call(block.merkleRoot).toString('hex');
